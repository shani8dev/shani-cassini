...
import shani_cassini._httpx_compat  # noqa: F401  — must precede `import httpx`
...
class APIClient:
    ...
    def get_gateway_status(self) -> Dict[str, Any]:
        """Get gateway status from Pulsar OS Sayri.

        Returns:
            Gateway status response containing connected gateways and their status
        """
        logger.info("Fetching gateway status")
        try:
            response = self._client.get(
                f"{self._base_url}/gateway/status",
                headers=self._get_headers()
            )
            return self._handle_response(response)
        except Exception as e:
